<template>
  <section class="card app-panel live-monitor-panel">
    <div class="module-title">
      <div class="module-title-left">
        <span class="module-title-emoji" aria-hidden="true">{{ titleEmoji }}</span>实时监控
      </div>
      <button
        type="button"
        class="mute-toggle-btn"
        :aria-pressed="!isMuted"
        :title="isMuted ? '开启声音' : '静音'"
        @click="toggleMute"
      >
        <img
          class="mute-toggle-btn__icon"
          :class="isMuted ? 'is-muted' : 'is-sound'"
          :src="isMuted ? muteIconSrc : soundIconSrc"
          width="17"
          height="17"
          alt=""
          aria-hidden="true"
          draggable="false"
        />
      </button>
    </div>

    <div class="channel-row" role="tablist" aria-label="摄像头机位">
      <button
        v-for="cam in cameras"
        :key="cam.id"
        type="button"
        class="channel-tab"
        :class="{ 'is-active': cam.id === activeId }"
        role="tab"
        :aria-selected="cam.id === activeId"
        @click="selectCam(cam.id)"
      >
        {{ cam.label }}
      </button>
    </div>

    <div class="player-wrap">
      <iframe
        v-if="embedSrc"
        :key="`${activeId}-${isMuted ? 'muted' : 'sound'}`"
        class="player-iframe"
        title="YouTube 实时摄像头"
        :src="embedSrc"
        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
        allowfullscreen
        referrerpolicy="strict-origin-when-cross-origin"
      />
    </div>
  </section>
</template>

<script>
/**
 * 实时监控面板：
 * 展示多城市机位直播，支持一键静音并统一 YouTube embed 参数。
 */
import { PANEL_TITLE_EMOJI } from "./js/panelTitleEmoji.js";
import { buildYoutubeVideoEmbedUrl } from "./js/liveEmbed.js";

/** YouTube 固定视频/直播 embed，与 LiveNewsPanel 参数风格一致 */
const CAMERAS = [
  // 实时监控数据源（YouTube embed）
  { id: "jerusalem", label: "Jerusalem", videoId: "77akujLn4k8" },
  { id: "washington", label: "washington", videoId: "1wV9lLe14aU" },
  { id: "paris", label: "Paris", videoId: "OzYp4NRZlwQ" },
  // 你的链接包含 t=11s，这里把起播时间传给 embed
  { id: "taibei", label: "Taibei", videoId: "z_fY1pj1VBw"}
];

export default {
  name: "LiveMonitorPanel",
  data() {
    return {
      titleEmoji: PANEL_TITLE_EMOJI["live-monitor"],
      cameras: CAMERAS,
      activeId: CAMERAS[0].id,
      isMuted: true,
      soundIconSrc: `${process.env.BASE_URL}assets/svg/声音播放.svg`,
      muteIconSrc: `${process.env.BASE_URL}assets/svg/声音静音.svg`
    };
  },
  computed: {
    activeCam() {
      return this.cameras.find((c) => c.id === this.activeId) || this.cameras[0];
    },
    embedSrc() {
      const cam = this.activeCam;
      return cam && cam.videoId ? buildYoutubeVideoEmbedUrl(cam, this.isMuted) : "";
    }
  },
  methods: {
    /** 切换监控机位。 */
    selectCam(id) {
      if (id === this.activeId) return;
      this.activeId = id;
    },
    /** 切换播放器静音状态。 */
    toggleMute() {
      this.isMuted = !this.isMuted;
    }
  }
};
</script>

<style scoped>
.live-monitor-panel {
  height: 100%;
  min-height: 0;
  overflow: hidden;
  display: flex;
  flex-direction: column;
}

.module-title {
  display: flex;
  align-items: center;
  justify-content: flex-start;
  gap: 6px;
}

.module-title-left {
  display: inline-flex;
  align-items: center;
}

.mute-toggle-btn {
  width: 22px;
  height: 22px;
  padding: 0;
  border: none;
  border-radius: 4px;
  background: transparent;
  cursor: pointer;
  line-height: 0;
  display: inline-flex;
  align-items: center;
  justify-content: center;
  flex-shrink: 0;
  overflow: visible;
}

.mute-toggle-btn:hover {
  opacity: 0.92;
}

.mute-toggle-btn__icon {
  width: 17px;
  height: 17px;
  display: block;
  object-fit: contain;
  object-position: center;
  flex-shrink: 0;
  pointer-events: none;
  transform-origin: center center;
}

.mute-toggle-btn__icon.is-sound {
  filter: brightness(0) saturate(100%) invert(52%) sepia(51%) saturate(520%)
    hue-rotate(95deg) brightness(96%) contrast(92%);
}

.mute-toggle-btn__icon.is-muted {
  filter: brightness(0) saturate(100%) invert(58%) sepia(4%) saturate(0%)
    hue-rotate(169deg) brightness(96%) contrast(88%);
  opacity: 0.9;
  transform: scale(1.14);
}

.channel-row {
  display: flex;
  flex-wrap: nowrap;
  gap: 4px;
  margin-top: 8px;
  flex-shrink: 0;
  overflow-x: auto;
  overflow-y: hidden;
  scrollbar-width: thin;
}

.channel-tab {
  padding: 4px 8px;
  border-radius: 6px;
  border: 1px solid rgba(120, 140, 165, 0.45);
  background: rgba(8, 18, 36, 0.55);
  color: rgba(220, 232, 248, 0.92);
  font-size: 10px;
  font-weight: 700;
  letter-spacing: 0;
  cursor: pointer;
  line-height: 1.2;
  flex: 0 0 auto;
  white-space: nowrap;
}

.channel-tab:hover {
  border-color: rgba(95, 185, 255, 0.55);
  color: rgba(235, 246, 255, 0.98);
}

.channel-tab.is-active {
  border-color: rgba(34, 197, 94, 0.85);
  background: rgba(22, 101, 52, 0.35);
  color: #fff;
}

.player-wrap {
  flex: 1;
  min-height: 0;
  margin-top: 10px;
  position: relative;
  border-radius: 8px;
  overflow: hidden;
  border: 1px solid rgba(95, 185, 255, 0.22);
  background: #0a0f18;
}

.player-iframe {
  display: block;
  width: 100%;
  height: 100%;
  min-height: 160px;
  border: 0;
}
</style>
