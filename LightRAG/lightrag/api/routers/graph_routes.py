...
def create_graph_routes(rag, api_key: Optional[str] = None):
    ...
    async def get_knowledge_graph(
        ...
        top_k: Optional[int] = Query(
            None,
            description="Alias for max_nodes (API consistency with /graph/query). If set, overrides max_nodes.",
            ge=1,
            le=1000,
        ),
        chunk_top_k: Optional[int] = Query(
            None,
            description="Not applicable to graph traversal. Reserved for API consistency.",
        ),
    ):
        """
        ...
        Args:
            ...
            top_k: Alias for max_nodes. If set, overrides max_nodes.
            chunk_top_k: Reserved for API consistency with /graph/query, not used in graph traversal.
        ...
        try:
            ...
            # top_k overrides max_nodes when set (API consistency)
            effective_max_nodes = top_k if top_k is not None else max_nodes
    ...
    @router.get("/graph/query", dependencies=[Depends(combined_auth)])
    async def query_knowledge_graph(
        query: str = Query(..., description="Event description for graph retrieval"),
        top_k: int = Query(5, description="Number of seed entities to retrieve and expand via graph traversal", ge=1, le=20),
        max_depth: int = Query(2, description="Graph traversal depth from each seed entity", ge=1, le=5),
        max_nodes: int = Query(50, description="Max total nodes in the final result", ge=1, le=2000),
    ):
        """
        Event-based knowledge graph retrieval.

        Combines semantic search (mix mode) + graph traversal:
        1. Uses semantic search (aquery_data, mix mode) to find entities relevant to the event
        2. Expands each seed entity's neighborhood via graph traversal
        3. Merges and deduplicates all subgraphs into one result

        Example:
            GET /graph/query?query=美军一架F-15E战斗机3日在伊朗上空被击落

        Args:
            query:事件，如"美军一架F-15E战斗机3日在伊朗上空被击落"
            top_k:取多少个种子实体做语义搜索+图遍历展开
            max_depth:每个种子实体的遍历深度
            max_nodes:最终结果总节点数上限

        Returns:
            KnowledgeGraph format: {
                "nodes": [{"id": str, "labels": list[str], "properties": dict}],
                "edges": [{"id": str, "type": str | None, "source": str, "target": str, "properties": dict}],
                "is_truncated": bool
            }
        """
        try:
            # Step 1: Semantic search for seed entities (always mix mode)
            search_result = await rag.aquery_data(
                query=query,
            ...
            # Step 2: Graph traversal from each seed entity (in parallel)
            node_budget = max(max_nodes // len(seed_entity_names), 10)
            ...
            all_nodes: list[dict] = []
            seen_edge_keys: set[tuple[str, str, Optional[str]]] = set()
            all_edges: list[dict] = []

            for sg in subgraphs:
                for node in (sg.nodes if hasattr(sg, "nodes") else sg.get("nodes", [])):
                ...
                for edge in (sg.edges if hasattr(sg, "edges") else sg.get("edges", [])):
                    src = edge.source if hasattr(edge, "source") else edge.get("source", "")
                    tgt = edge.target if hasattr(edge, "target") else edge.get("target", "")
                    e_type = edge.type if hasattr(edge, "type") else edge.get("type", None)
                    key = (src, tgt, e_type)
            ...
            logger.error(traceback.format_exc())
            raise HTTPException(
                status_code=500, detail=f"Error querying knowledge graph: {str(e)}"
            )
