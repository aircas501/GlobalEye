...
def create_app(args):
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            # RocketMQ consumer（仅在配置了 ROCKETMQ_NAMESRV_ADDR 时启动）
            if os.getenv("ROCKETMQ_NAMESRV_ADDR"):
                consumer = start_rocketmq_consumer(rag)
                if consumer is not None:
                    app.state.rocketmq_consumer = consumer
                    ASCIIColors.green(
                        f"RocketMQ consumer started: {os.getenv('ROCKETMQ_TOPIC', 'lightrag_content')}"
                    )
                else:
                    logger.warning(
                        "RocketMQ consumer 启动失败（依赖未安装），服务将继续运行但不会消费 MQ 消息"
                    )
            ...
            # Stop RocketMQ consumer
            consumer = getattr(app.state, "rocketmq_consumer", None)
            if consumer:
